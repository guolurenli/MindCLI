...
`/plan` 与 `/team` 是两个不同执行模式，不合并编排语义；二者只共享 `agent/plan/DependencyGraph.java` 里的中性 DAG 计算（拓扑顺序、就绪节点、批次和阻塞依赖诊断），依赖满足规则仍由各自模式按 Task/Step 状态传入。`/plan` 的同一 ready wave 只有明确 `FILE_READ` / 只读工具任务可以最多 4 路并发，修改型、命令型、分析/验证型和未知任务按原顺序串行；失败恢复按任务字段 `critical` / `degradation` 决策：`critical=false + degradation=SKIP` 才可跳过，`BLOCK` 直接失败，其余默认回退为局部重规划。`/team` 的 ready batch 现在会先按 step 指纹去重，重复步骤复用同一执行结果；只读 / 无工具分析步骤优先路由给 `EXPLORER` profile 执行，只允许 `@read` 工具组；声明 `execute_command`、未知工具或写入工具的步骤一律按修改型处理并交给 `WORKER`；编排器内建规划（无独立 planner 子代理）；同一 ready wave 内多个无依赖写入步骤各自创建独立 git worktree 并行执行，完成后先在临时 integration worktree 中统一合并；冲突时整批不更新主工作区，标记步骤失败并上报冲突文件清单；非 git 仓库、checkpoint 或 worktree 创建失败时回退为串行单步。执行者随后进入自己的 review->repair 循环，通过后才关闭步骤，底层 `ToolDispatcher` / `ResourceLockManager` 和 workspace 路径、命令策略仍作为最后一道防线。
...
Agent Runtime 账本默认通过 `RunStoreFactory` 写到 `~/.mindcli/runs`（可用 `mindcli.runs.dir` / `MINDCLI_RUNS_DIR` 改写），`InMemoryRunStore` 仅保留给测试和降级。JSONL ledger 是 source of truth：每个事件包含 run 内递增 `seq` 和唯一 `eventId`，`run.meta.json` / `run.state.json` 由事件投影生成；append 会单次加载当前 ledger，并复用同一事件快照完成 seq 分配、坏尾修复与派生文件投影，读取会忽略尾部坏行，继续 append 前会先截断坏尾；顶层 `runId` 与 child `parentRunId` 在读写路径解析前都必须通过安全字符校验。Plan 在审阅通过及局部重规划合并后写 `PLAN_DEFINED`，并在 task 状态变化后写 `PLAN_TASK_CHECKPOINT`；Team 在规划确定后写 `TEAM_PLAN_DEFINED`，execute/review child 启动前及 step 终态写 `TEAM_STEP_CHECKPOINT`。Team 恢复复用原 DAG 并跳过 `COMPLETED` / `FAILED` / `SKIPPED` step；child 工具请求与结果必须完整匹配，`REVIEWING`、`AWAITING_MERGE`、未完成调用或非终态成功副作用一律 fail closed。重复步骤用一条 checkpoint 原子确认，worktree 写入只有成功合并主工作区后才可落为 `COMPLETED`。AgentRuntime 可写 `SNAPSHOT_CREATED`，把 `PRE_RUN` / `POST_RUN` Side-Git checkpoint 与 runId 关联；CLI Agent 主路径不再额外套 `SnapshotService.runTurn(...)`，避免同一 run 产生旧 turn snapshot 与 runtime snapshot 两套快照；`/run inspect <runId>` 通过 `RunRecoveryService` 展示状态、checkpoint、风险和恢复提示。CLI 启动时只读发现最近 3 个具备恢复上下文的顶层父 run并写入 Banner note，不扫描 `children/`、不自动恢复，也不绕过 `/run resume` 的风险确认、HITL 或策略校验。Multi-Agent 的 explorer / worker 执行与自审写入 child run，事件带 `parentRunId`、`rootRunId`、`role`、`stepId`、`attempt`、`phase=execute|review`；parent `run.state.json` 会 materialize child run 摘要。自审调用失败、输出不可解析、重试后仍拒绝时必须 fail closed，不能把执行候选结果标记为完成；review phase 摘要保留 `approved` / `businessStatus`，供恢复和审计判断。
...
MCP 动态工具：`mcp__{server}__{tool}`（+ resources 虚拟工具）。协议与 stdio/Streamable HTTP 传输仅由官方 MCP Java SDK 2.0.1 提供，不保留自研 JSON-RPC / wire-protocol / transport fallback；MindCLI 只保留生命周期、命名空间、策略、审计、内容适配和资源缓存 facade。MCP 启动协调与官方 transport 创建位于 `capability/mcp/lifecycle/`，`McpServerManager` 继续作为对外 facade。server 禁用、重启、重新加载配置或关闭时，必须同时注销旧工具、关闭旧 client 并清理该 server 的 resource cache，避免失效资源继续进入 prompt 索引。
...
应用级运行配置统一通过 `platform/config/ConfigValueResolver` 读取，优先级固定为 `JVM system property > OS environment > 项目 .env > 用户 ~/.env > 默认值`；LLM HTTP 超时使用 `mindcli.llm.{connect,read,write,call}.timeout.seconds` 与对应的 `MINDCLI_LLM_*_TIMEOUT_SECONDS`，默认值为 `60/300/60/600` 秒；新增配置不要在业务模块重复实现 property/env/.env 解析。`user.home` / `user.dir` / `os.name`、JVM 编码和 `TERM` / `COLORTERM` / `COLUMNS` / `NO_COLOR` 等运行环境探测保留直接读取。

JSONL ledger 的 append 使用每个 `run.jsonl` 对应的 `run.jsonl.lock` 文件锁，保护跨进程/跨实例的 seq 分配、坏尾修复和写入；JVM 内的 `ReentrantLock` 按规范化 ledger 路径隔离，不同 run 不共享实例级 monitor；Runtime API 同一 thread 的 turn 按提交顺序串行执行，不同 thread 仍可并行。

Runtime API 的 SQLite 连接必须设置 5 秒 `busy_timeout`；创建 thread 与 `thread.created` 事件使用同一事务，避免 thread 记录和首个事件只写入一半。
...
- `LineReader` 使用 `app/cli/interaction/MindCliHistory` 持久化输入历史到 `~/.mindcli/history/input.history`；如果 `mindcli.history.file` / `MINDCLI_HISTORY_FILE` 指向目录，也会自动使用该目录下的 `input.history`，避免把目录当文件读；默认忽略空白、重复、明显密钥/Bearer、base64 图片和超长输入，用户可用 `/history clear` 清空本机输入历史。JLine autosuggestion/autopair、Ctrl+O 折叠、Ctrl+V 剪贴板图片和 ESC 清空输入的 widget 统一由 `app/cli/interaction/CliInteractiveWidgets` 注册，raw terminal 字节读取与 ESC 取消监听由 `app/cli/interaction/CliTerminalInput` 负责，`Main` 只保留兼容入口。
...
- `ToolResourceClassifier` 同时推导副作用类别与资源锁：文件读 shared、文件写 exclusive，文件访问补充祖先目录 shared 锁；`list_dir` 语义上只读，但对目标目录加 exclusive 锁，使目录枚举与该目录下文件写入互斥；长期记忆读取 shared、`save_memory` exclusive；workspace 命令默认修改型 + exclusive，只有内置白名单命令为只读 + shared，含管道、重定向、命令连接符或外部 diff / output 选项时降级为修改型；MCP 只有官方 annotation 满足 `readOnlyHint=true && destructiveHint!=true` 时为只读 + shared，缺失/矛盾注解以及未知工具默认串行 + exclusive
...
- `ToolDispatcher` 把同一 LLM turn 视为无数据依赖：非审批只读工具先以最多 4 路并发执行，修改型、未知和需要 HITL 的工具随后按原相对顺序逐个执行；结果仍按原始 `tool_call` 索引返回。同轮多个 `write_file` 指向同一规范化文件时，这些写入均不执行并返回 `TOOL_BATCH_CONFLICT`，要求模型下一轮合并为一次最终写入。dispatcher 会把 run 的 `approvalPolicy` 显式绑定到实际工具工作线程并在 `finally` 清理
- 语义调度不替代资源锁：前者控制单轮调用策略，`ResourceLockManager` 继续防止同 JVM 内跨 dispatcher / task / run 的物理竞争；锁不覆盖两个独立 MindCLI 进程同时修改同一工作区
...
| DAG/Plan / 恢复 | `mvn test -Dtest=ExecutionPlanTest,PlanExecuteAgentTest,PlanCheckpointCodecTest,RunRecoveryServiceTest,AgentRuntimeTest,PlanExactResumeEvalTest,TeamExactResumeEvalTest` |
| Multi-Agent | `mvn test -Dtest=AgentRoleTest,AgentMessageTest,SubAgentTest,AgentProfileLoaderTest,AgentOrchestratorTest` |
| Agent 结果评测 | `mvn test "-Dtest=com.mindcli.eval.*Test" -DskipTests=false`（17 个离线场景 + 1 个 fixture smoke，不需要 API Key） |
...
- `/run resume <runId>` 已支持安全重入：仅接受有原始输入且状态为 `RESUMABLE` 的 run，继续使用现有策略/HITL；包含已知写入、命令或 MCP 调用时，必须追加 `--confirm`。ReAct 按消息/工具调用边界恢复并复用已完成结果；Plan 恢复原 DAG 的安全 task；Team 恢复原 DAG 的安全 step，跳过终态 step，不重新规划。旧或损坏 checkpoint、非终态成功副作用、不完整 child 调用、`REVIEWING` 与 `AWAITING_MERGE` 都不会追加 `RUN_RESUMED`，即使 `--confirm` 也必须人工检查。
- 后续恢复工作只包含 child 内部 LLM/tool loop 断点续跑、崩溃遗留 worktree 自动接管和跨机器恢复；启动期继续只做候选发现，不自动执行恢复。
