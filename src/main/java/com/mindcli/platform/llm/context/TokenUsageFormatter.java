...
public final class TokenUsageFormatter {
    ...
    public static String format(LlmClient llmClient, long inputTokens, long outputTokens,
                                long cachedInputTokens, long startNanos) {
    ...
    public static String estimatedCostCny(LlmClient llmClient, long inputTokens, long outputTokens, long cachedInputTokens) {
        ...
        long cached = Math.max(0L, Math.min(inputTokens, cachedInputTokens));
        long uncachedInput = Math.max(0L, inputTokens - cached);
        double cny = (uncachedInput / 1_000_000.0) * inputPerMillion
                + (cached / 1_000_000.0) * cachedPerMillion
                + (Math.max(0L, outputTokens) / 1_000_000.0) * outputPerMillion;
