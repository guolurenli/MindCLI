...
- 日期：2026-08-17
- 状态：已确认（待实现）
- 范围：全局 UI 配色统一到启动猫耳女仆图的暖色调，去掉冷调科技感
...
- 不改动任何文本内容、布局、光标逻辑、JLine dock 结构或启动图渲染尺寸，只改颜色。
- 不引入第三方取色/渲染依赖。

## 6. 验证

1. `mvn test` 通过（重点：`AnsiStyle`、`BottomStatusBar` 相关测试若有颜色断言需同步更新）。
2. 启动 CLI（`NO_TUI=true`）确认：猫耳图正常显示、标题奶油白、提示暖灰、无青色/绿色/亮黄残留。
